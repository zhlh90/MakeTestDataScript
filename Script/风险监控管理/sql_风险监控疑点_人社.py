...
def make_sql(country_code, n):
    ...
    if not isExists:
        # 如果不存在则创建目录
        os.mkdir(path)
    file_name = path + 'TEST001'+str(otherStyleTime3)+'.sql'
    ...
    while (n > 0):
        ...
        line = str_head_zhu + '"' + FIELD1 + '", "' + FIELD2 + '", "' + FIELD3 + '", "' + ID + '", TO_TIMESTAMP(\'' + LAST_MODIFIED_DATE + '\',\'SYYYY-MM-DD HH24:MI:SS:FF6\'), "' + INSURANCE_CODE + '");'
