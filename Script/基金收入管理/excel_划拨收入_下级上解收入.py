...
def make_excel(agency_code, insurance_code, n):
    ...
    headers = ['SUBORDINATE_UNIT', 'RECEIVE_UNIT', 'RECEIPT_ACCOUNT_NAME', 'RECEIPT_ACCOUNT_NO', 'RECEIPT_ACCOUNT_TYPE', 'BANK_CODE', 'SUBSIDY_AMOUNT', 'INSURANCE_CODE', 'AGENCY_CODE', 'REGION_CODE']

    rows = []

    no = 1
    while (n > 0):
        #上解经办机构银行账户名称（根据实际数据变更）
        SUBORDINATE_UNIT = '测试上解付款单位' + ran_strx("0123456789", 8)
        #上解收款往来单位名称（根据实际数据变更）
        RECEIVE_UNIT = '测试收款单位'
        #上解收款往来单位银行账户名称（根据实际数据变更）
        RECEIPT_ACCOUNT_NAME = '测试收款单位账户'
        #上解收款往来单位银行账号（根据实际数据变更）
        ...
        row =[SUBORDINATE_UNIT, RECEIVE_UNIT, RECEIPT_ACCOUNT_NAME, RECEIPT_ACCOUNT_NO, RECEIPT_ACCOUNT_TYPE, BANK_CODE, SUBSIDY_AMOUNT, INSURANCE_CODE, AGENCY_CODE, REGION_CODE]
        rows.append(row)
        no += 1
        n -= 1
